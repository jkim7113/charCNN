...
def create_model(filter_kernels, dense_outputs, maxlen, vocab_size, nb_filter, cat_output):
    ...
    conv = Conv1D(filters=nb_filter, kernel_size=3, kernel_initializer=initializer,
                         padding='valid', activation='relu',
                         input_shape=(maxlen, vocab_size))(embedded)
    # conv = MaxPooling1D(pool_size=2)(conv)

    conv1 = Conv1D(filters=nb_filter, kernel_size=3, kernel_initializer=initializer,
                          padding='valid', activation='relu')(conv)
    conv1 = MaxPooling1D(pool_size=2)(conv1)

    conv2 = Conv1D(filters=nb_filter, kernel_size=2, kernel_initializer=initializer,
                          padding='valid', activation='relu')(conv1)

    conv3 = Conv1D(filters=nb_filter, kernel_size=2, kernel_initializer=initializer,
                          padding='valid', activation='tanh')(conv2)
    conv3 = MaxPooling1D(pool_size=2)(conv3)
    
    conv4 = Conv1D(filters=nb_filter, kernel_size=2, kernel_initializer=initializer,
    ...
    conv5 = Conv1D(filters=nb_filter, kernel_size=2, kernel_initializer=initializer,
                          padding='valid', activation='relu')(conv3)
